...
export default function ServiceSocialLabPage(props:ServiceModel) {
  ...
  const items = [
    {
      title:"Workshops y acciones educativas",
      desc:"Experiencias para desarrollar la propia perspectiva, aprender a construir una historia para activar el público objetivo y difundir el trabajo en el territorio."
    },
    {
      title:"Charlas y seminarios",
      desc:"Generamos un diálogo y debate compartiendo buenas prácticas y experiencias a través de la fotografía."
    },
    {
      title:"Desarrollo de contenidos educativos",
      desc:"Capacitación habilidades técnicas y creativas para movilizar a la ciudadanía difundiendo problemáticas socioambientales en formatos impresos y digitales."
    }
  ];
      ...
      <div className="row" key={index}>
        <div className="col-12 col-lg-12 text-left">
          <div className="title-desc">
            <h2 className="display-4 display-title anim-2 decor">{item.title}</h2>
            <p className="anim-2" dangerouslySetInnerHTML={{__html: item.desc}} />
          </div>
        </div>
      </div>
    )
